...
function director:addParamsForModule(moduleName, params)
	savedParams[moduleName] = params
end
...
local function loadScene ( moduleName, target )
	...
	-- Prev
 	if string.lower(target) == "curr" then
 		...
 		--
 		local params = savedParams[moduleName]
		currScreen = require(moduleName).new(params)
	...
	-- Next
		...
		--
 		...
 		--
 		local params = savedParams[moduleName]
		nextScreen = require(moduleName).new(params)
